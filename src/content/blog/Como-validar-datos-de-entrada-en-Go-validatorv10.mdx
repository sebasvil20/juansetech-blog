...
- `min` y `max` tienen diferentes comportamientos dependiendo del tipo de dato al que se lo apliques:
  - Para números: válida que el valor sea mayor o igual al puesto
  - Para strings: válida que la longitud sea mínimo el valor pasado
  - Para slices, arrays y mapas: válida la cantidad de items en ellos.
