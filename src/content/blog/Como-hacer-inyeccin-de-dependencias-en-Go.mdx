...
El repositorio con todos los archivos lo tienes aca: [sebasvil20/blog-di](https://github.com/sebasvil20/blog-di 'sebasvil20/blog-di')
...
Además del ciclo de vida, te deja organizar tus dependencias en "Módulos" que no son más que grupos de dependencias, así definimos un módulo para fx (En la carpeta de providers cree un file llamado `books_module`):
...
Ahora en el app.go podemos llamar al módulo e inicializar el resto de cosas que necesitemos. Además, como mencioné anteriormente, fx te deja controlar el ciclo de vida de las dependencias y es al menos necesario que definas el hook onStart. El archivo app.go quedaría asi:
...
Hey! 😁 ¡Has llegado al final! Si te ha gustado el artículo o te ha sido útil, no dudes en compartirlo, puedes decirme tus opiniones a través de mi cuenta de [X / Twitter.](https://twitter.com/JuanseTech 'X / Twitter')
